...
use crate::{bundle_fs::FS, file_parsers::rs::parse_rs};

/// Extract files to disk matching a glob pattern
pub fn dump_rs(fs: &mut FS, patterns: &[Pattern], output_folder: &Path) -> Result<()> {
    for pattern in patterns {
        ensure!(
            pattern.as_str().ends_with(".rs"),
            "Only .rs export is supported."
        );
    }
