...
use crate::{bundle_fs::FS, file_parsers::tsi::parse_tsi};

/// Extract files to disk matching a glob pattern
pub fn dump_tsi(fs: &mut FS, patterns: &[Pattern], output_folder: &Path) -> Result<()> {
    for pattern in patterns {
        ensure!(
            pattern.as_str().ends_with(".tsi"),
            "Only .tsi map export is supported."
        );
    }
